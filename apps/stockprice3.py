...
# 2. Save as JSON
with open("chat_messages1.json", "w", encoding="utf-8") as json_file:
    json.dump(formatted_chat_messages, json_file, ensure_ascii=False, indent=4)

# 1. JSONファイルを読み込む
with open("chat_messages1.json", "r", encoding="utf-8") as json_file:
    data = json.load(json_file)

# 2. defaultdictを生成して、読み込んだデータをコピー
# defaultdictのデフォルト値を空のリストに設定しています
...
# 2. Save as JSON
with open("chat_messages2.json", "w", encoding="utf-8") as json_file:
    json.dump(formatted_chat_messages, json_file, ensure_ascii=False, indent=4)
